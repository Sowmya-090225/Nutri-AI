...
/**
 * Common logic to update or initialize a user's streak
 * @param {string} userId
 */
async function updateStreak(userId) {
  ...
  let streakRecord = await Streak.findOne({ where: { userId } });
  ...
  if (!streakRecord) {
    ...
    return { count: streakRecord.count, message: '🔥 You started a new streak today!' };
  }
  ...
  if (diffDays === 0) {
    ...
    message = `🔥 You're on a ${streakRecord.count}-day streak!`;
  } else if (diffDays === 1) {
  ...
  return { count: streakRecord.count, message };
}
